...
kernel void r_896_8_4_4_512(device float* data0_14336, device float* data1_4096, device float* data2_58720256, uint3 gid [[threadgroup_position_in_grid]], uint3 lid [[thread_position_in_threadgroup]]) {
  ...
  *(acc0+3) = 0.0f;
  for (int Ridx0 = 0; Ridx0 < 512; Ridx0++) {
    ...
    int alu5 = (alu0+(lidx0*14336)+(Ridx0*114688));
  ...
  *(acc1+3) = 0.0f;
  ...
  bool alu23 = (lidx0==0);
  if (alu23) {
    *(data0_14336+alu0) = (*(acc1+0));
  }
  if (alu23) {
    *(data0_14336+(alu0+4)) = (*(acc1+1));
  }
  if (alu23) {
    *(data0_14336+(alu0+8)) = (*(acc1+2));
  }
  if (alu23) {
    *(data0_14336+(alu0+12)) = (*(acc1+3));
  }
}
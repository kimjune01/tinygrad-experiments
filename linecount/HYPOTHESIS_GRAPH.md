...
Applied the onnx.py protobuf parser refactor to the target codebase.

| Metric | Before | After | Delta |
|--------|--------|-------|-------|
| onnx.py tokenized lines | 1,046 | 1,017 | **-29** |
| sz.py total lines | 23,364 | 23,335 | **-29** |
| Headroom (of 24,000) | 636 | 665 | **+29** |

**Trajectory**: Divergent improvement on the only metric (tokenized line count). No regressions.

**Verification**:
- Import: OK
- Smoke test (real tinygrad PBBufferedReader + Tensor): OperatorSetIdProto, TensorShapeProto — PASS
- Mock validation (12/12 test cases): PASS
- Full ONNX test suite: requires onnxruntime (not installed locally), deferred to CI

**Classification**: Confirmed. Proceed to Phase 7 (bug hunt).

---

## Phase 7: Bug Hunt Results

### Round 1: Codex (structural)

Zero correctness bugs. Flagged two extensibility concerns:
1. String-prefix dispatch (`startswith("_parse_")`) is fragile if the table is extended incorrectly.
2. List-append semantics are implicit (inferred from default value type, not explicit metadata).

Both are about future misuse, not current behavior.

### Round 1: Gemini (logic tracing)

Zero bugs. Traced all seven message types, verified:
1. List copy via `list(v)` in the defaults comprehension prevents aliasing — each call gets its own `dim` list.
2. `_SIMPLE_PROTOS` as a class variable is safe — `_parse_proto` treats `defaults` as read-only.
3. `getattr` dispatch uses strict namespace segregation — shadowing is impossible because `self` and `self.reader` are never both checked for the same string.
4. Append semantics are safe because none of the reader/parser methods return lists — `obj.get(name)` can only be a list if explicitly configured in `defaults`.

### Convergence

Both reviewers: zero new findings. Codex's fragility warnings are moot under the current schema constraints (Gemini's analysis). No kill condition generated. **Converged.**
...
|----|-------|--------|--------|------------|
| [#16085](https://github.com/tinygrad/tinygrad/pull/16085) | onnx: deduplicate simple proto parsers | `onnx-proto-dedup` | **Open** | H₁ₐ (onnx refactor, -29 lines) |
| [#16070](https://github.com/tinygrad/tinygrad/pull/16070) | add Ops.WARP_REDUCE for GROUPTOP reductions | `warp-reduce-v2` | **Draft** | Motivating feature (+52 lines) |

The onnx PR is the line-budget offset for WARP_REDUCE. Both are on `kimjune01/tinygrad`.

---

## Summary

The codebase is clean. There is no dead code, minimal textual duplication, and the line budget is a social ratchet that gets raised when needed (9 times in 2025). The realistic line-reduction surface is ~45-50 lines from two concrete refactors (onnx protobuf parser -29, HCQ init dedup ~15-20), which partially offsets WARP_REDUCE's +52 lines.
...
**Recommendation**: For an external contributor, the onnx.py protobuf refactor is the highest-leverage PR: -29 tokenized lines (measured), low risk, aligns with #15717, and offsets more than half of WARP_REDUCE's +52.
